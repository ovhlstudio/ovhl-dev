...
get_structure_summary() {
    ...
    # Filter ketat: Hanya cari di dalam SCAN_DIRS (src, tests)
    # Exclude Packages, .git, _Index
...
get_file_distribution() {
    ...
    # Gunakan SCAN_DIRS loop agar konsisten dan tidak ngebaca folder root lain
    for scan_dir in "${SCAN_DIRS[@]}"; do
        local search_path="$root/$scan_dir"
        if [ ! -d "$search_path" ]; then continue; fi
        
        # Loop file bersih (Exclude Packages)
