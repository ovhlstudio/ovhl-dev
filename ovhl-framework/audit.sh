...
analyze_smart_packages() {
    ...
    for pkg_path in "$PACKAGES_DIR"/*; do
        ...
        # [FIX] Regex Updated: Also check for Loader.Pkg("name")
...
analyze_path_management() {
    ...
    # [FIX] Check for Loader Proxy as Path Resolver
    if find "${TARGET_DIRS[@]}" -type f \( -name "*.lua" -o -name "*.luau" \) -exec grep -l "Loader.*Proxy\|createProxy\|Loader\.Core" {} \; | grep -q .; then
    ...
    # [FIX] Check for Path Aliasing via Loader
    if find "${TARGET_DIRS[@]}" -type f \( -name "*.lua" -o -name "*.luau" \) -exec grep -l "Loader\.Core\|Loader\.UI\|Loader\.Pkg" {} \; | grep -q .; then
...
analyze_ui_system() {
    ...
    # [FIX] Enhanced Detection for Facade/Loader Patterns
    if find "${TARGET_DIRS[@]}" -type f \( -name "*.lua" -o -name "*.luau" \) -exec grep -l "Loader.Pkg.*Fusion\|API\.Fusion\|API\.New" {} \; | grep -q .; then
    ...
    if find "${TARGET_DIRS[@]}" -type f \( -name "*.lua" -o -name "*.luau" \) -exec grep -l "Loader.Pkg.*onyx\|API\.Onyx\|API\.Themer" {} \; | grep -q .; then
        onyx_detected=true
    ...
    ui_files=$(find "${TARGET_DIRS[@]}" -type f \( -name "*.lua" -o -name "*.luau" \) -exec grep -l "ScreenGui\|TextLabel\|TextButton\|ImageButton\|Frame\|GridContainer\|SmartWindow" {} \; | wc -l)
