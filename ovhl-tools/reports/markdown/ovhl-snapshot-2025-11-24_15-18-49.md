...
**Mandatory Rules:**
- Singleton Service (Server) & Controller (Client)
- Never use wait(). Use task.wait()
- Never use script.Parent.Parent. Use Loader.Module() or Context

### 2. 🧪 TESTING MANDATE
Every feature MUST have a Unit Test in tests/ folder
...
### 📂 Rojo Architecture Mapping
> Check the **Rojo Path** in each file snapshot for exact virtual location.
> The Snapshot section uses the real `default.project.json` mapping logic.
