...
- **Lifecycle:** `Init(ctx)` (Setup) -> `Start()` (Runtime).
- **Forbidden:**
  - ❌ Never use `wait()`. Use `task.wait()` or Signals.
  - ❌ Never use `script.Parent.Parent`. Use **Loader**.
  - ❌ Never execute runtime logic in `Init`.

### 2. 🧪 TESTING MANDATE (TestEZ)
- **Rule:** Every feature MUST have a Unit Test.
- **Location:** `tests/` directory (Mirror structure).
- **Example:** `src/Modules/Inventory.luau` -> `tests/Modules/Inventory.spec.luau`.
...
### ⚠️ Potential Global Mutable State (-5 pts)
...
### ⚠️ Hardcoded Service Path (-5 pts)
> **AI Advice:** Direct access to game services breaks modularity. Use the **Loader** or **Context** injection.
...
✅ No breaking changes detected in SharedConfig contracts.
...
> This mapping is generated from `default.project.json`. Use these paths for requires.

**(Note: Dynamic Mapping will be injected in V11. Showing Standard Structure for now)**
- **Server Logic:** `src/ServerScriptService/OVHL` -> `game.ServerScriptService.OVHL`
- **Client Logic:** `src/StarterPlayer/StarterPlayerScripts/OVHL` -> `game.StarterPlayer...OVHL`
- **Shared Logic:** `src/ReplicatedStorage/OVHL` -> `game.ReplicatedStorage.OVHL`
- **Unit Tests:** `tests/` -> Check Rojo Config for specific mount points.
